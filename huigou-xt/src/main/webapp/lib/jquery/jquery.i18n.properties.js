...
(function ($) {
     ...
     */
    $.i18nProp=function(){
    	var value=null,args = arguments,key='';
    	...
    	try{
			value=$.i18n.prop.apply(window,args);
    		return value;
    	}catch(e){
    		return key;
    	}
    };
    $.i18nPropAsDefault=function(code,defaultValue){
    	var value=defaultValue;
    	try{
			value=$.i18n.prop.call(window,code);
    		return value;
    	}catch(e){
    		return defaultValue;
    	}
...
/*****
 * xx modif 插件本身采用ajax 同步调用的方式实现页面初始化时读取国际化资源，考虑执行效率及ajax同步调用可能被取消的问题，固修改插件读取资源的方式。
 * 实现方式类似jsonp方式访问后台，读取资源后前台解析存储.
 * *****/
...
//jsonp默认回调方式
function i18nParseData(data){
	var unicodeRE = /(\\u.{4})/ig;
	try{
		/** Process key & value 只实现了插件中map的方式*/
		$.each(data,function(name,value){
	        ...
	        // add to map 单引号转义 替换为 ''
	        $.i18n.map[name] = value.replace(/\'/g,"''");
		});
	}catch(e){}
}
