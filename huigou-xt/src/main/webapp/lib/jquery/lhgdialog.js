...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
		...
		that.config = config;
		//按钮点击后是否延迟启用 默认延迟
		that.buttonDelay=config.delay===false?false:true;
		...
		if(config.show){
			$('table.ui_border',that.DOM.wrap).show();
		}
		...
		//add 滚动时关闭弹出层
		$(DOM.content).on('scroll',function(){
	...
	button: function()
	{
		...
		for( ; i < ags.length; i++ )
		{
			...
			focusButton='btn-success';
			button = !isNewButton ? listeners[id].elem : _doc.createElement('button');
			$(button).addClass('btn').addClass('btn-primary').attr('type','button');
			...
			{	
			    that._focus && that._focus.removeClass(focusButton);
				that._focus = $(button).removeClass('btn-primary').addClass(focusButton);
				that.focus();
			}
	...
	size: function( width, height )
	{
		...
		var contentHeight=DOM.content.outerHeight(true);
		...
		if( typeof height === 'number' ){
			if(contentHeight > windowHeight-60){
				DOM.content.addClass('dom-overflow-auto').height(windowHeight-70);
			}
			height = height + 'px';
		}else{
			//控制内容高度不超过窗口高度
		...
		if (!$.support.leadingWhitespace){
			//处理 ie8 font 无法显示问题
			DOM.content.find('i.fa').hide();
			setTimeout(function(){
				DOM.content.find('i.fa').show();
			},0);
		}
		style.width = width;
		style.height = height;
		DOM.content.width(width);
	...
	position: function( left, top )
	{
		...
		/***
		 * iphone在iframe页面中获取$(document).scrollTop()始终未空
		 * 这里判断如果是iphone --$.isIosPhone 
		 * 获取iframe包裹页面的ScrollTop
		 ***/
		if($.isIosPhone&&docTop==0) {
			docTop = $.windowScrollTop();
			docLeft = $.windowScrollLeft();
		}
		
		var dl = isFixed ? 0 : docLeft,
			dt = isFixed ? 0 : docTop,
			ww = _$top.width(),
			wh = _$top.height(),
			ow = wrap.offsetWidth,
	...
	show: function( args )
	{
		...
		$('table.ui_border',this.DOM.wrap).show();
	...
	hide: function( args )
	{
		$('table.ui_border',this.DOM.wrap).hide();
	...
	unlock: function()
	{
		...
		if( mask && that._lock )
		{
			...
			{
				var _maskIndex=$(mask).data('zIndex');
				if($.isNumeric(_maskIndex)){
					mask.style.zIndex = _maskIndex;
				}else{
					mask.style.display = 'none';
				}
			}
			$(mask).removeData('zIndex');
	...
	_iframe: function( url )
	{
	    var that = this, iframe, $iframe, iwin, $idoc, $ibody, iWidth, iHeight,
		    ...
		    initCss = 'position:absolute;left:-9999em;top:0;border:none 0;background:transparent;width:100%;height:100%;display:none;',
		...
		$iframe=$('<iframe style="'+initCss+'" frameborder="0"></iframe>');	
		$iframe.attr("name", config.id);
		iframe = that.iframe = $iframe[0];
		//IE浏览器IFRAME加载 先加载到body加载完成后 在放入对话框
		var _ie9=$.browser.msie&&$.browser.version<=9;
		$iframe.appendTo(_ie9?'body':$content); 
		...
		var load = that._fmLoad = function()
		{
			...
			// 适应iframe尺寸
			setTimeout(function(){
				$(iframe).unbind('load',that._fmLoad);
				iframe.style.cssText = loadCss;
				//IE 浏览器下 将iframe 移动到dialog
				if (_ie9){
					$(iframe).appendTo($content);
				}
				/*iphone在iframe页面的宽度不受父页面影响，避免撑开页面*/
				if($.isIosPhone){
					var ifrBody=$($iframe.prop('contentWindow').document.body);
                	//重新修改body长宽值
                	that.size(iWidth,iHeight);
                	ifrBody.css({width:$content.width(),height:$content.height()});
				}
			},200);// setTimeout: 防止IE6~7对话框样式渲染异常
		};
	...
	_addEvent: function()
	{
		...
		DOM.wrap.bind('click', function(event){
			...
			else
			{
				...
				//2018-05-16 增加延时启用
				if(callbackID){
					if( $(target).hasClass('disabled') ){return false;}//禁用按钮点击无效
					if(callbackID=='cancel'){
						that._click(callbackID);
					}else{
						that._click(callbackID);
						if(!that.buttonDelay){
							return;
						}
						that._delayDisable(target,callbackID);
					}
				}
			}
	...
	_delayDisable:function(btn,callbackID){
		var that = this;
		//点击后禁用按钮， 默认延时2秒启用
		$(btn).addClass('disabled');
		that[callbackID+'delayTimes']=setTimeout(function(){
...
lhgdialog.setting =
{
	...
	title: 'common.dialog.title',// 标题,默认'视窗'
	button: null,	     		// 自定义按钮
	ok: null,					// 确定按钮回调函数
	cancel: null,				// 取消按钮回调函数
	init: null,					// 对话框初始化后执行的函数
	close: null,				// 对话框关闭前执行的函数
	okVal: 'common.button.ok',	// 确定按钮文本,默认'确定'
	cancelVal: 'common.button.cancel',// 取消按钮文本,默认'取消'
	skin: '',					// 多皮肤共存预留接口
	esc: true,					// 是否支持Esc键关闭
...
var _zIndexAsMask = function()
{
	var mask = $('#ldg_lockmask',lhgdialog.doc);
	if(mask.length > 0){
		if(mask.is(':visible')){
			mask.data('zIndex',mask[0].style.zIndex);
		}
	}
};
/**
