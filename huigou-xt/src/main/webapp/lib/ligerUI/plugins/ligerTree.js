...
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        {
        	if($.isFunction(this.options.isLeaf)){
        ...
        {  
            ...
            $("." + g._getParentNodeClassName(), treeNode).each(function (){
                $(this).removeClass(g._getParentNodeClassName()).addClass(g._getChildNodeClassName());
            });
        },
        ...
        _upadteTreeWidth: function ()
        {
            ...
            //xx 修改width:'100%' 树结构中的节点信息显示不完整
            g.tree.css({minWidth:treeWidth,width:'100%'});
        ...
        _getNodeClassName: function (o)
        {
            var g = this, p = this.options; 
            var nodeClassName = o.nodeClassName != null ? o.nodeClassName : p.nodeClassName;
            if (nodeClassName == null) return false; 
            if (typeof (nodeClassName) == "function") nodeClassName = nodeClassName.call(g,o);
            if (typeof (nodeClassName) == "string") return nodeClassName; 
            return nodeClassName;
        },
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                var nodeClassName=g._getNodeClassName(o);
                if(nodeClassName){
                	treehtmlarr.push('<span class="',nodeClassName,'">');
                }else{
                	treehtmlarr.push('<span>');
                }
                if (p.render)
                {
                    treehtmlarr.push(p.render(o, o[p.textFieldName]));
                } else
                {
                    treehtmlarr.push(o[p.textFieldName]);
                }
                treehtmlarr.push('</span>');
                treehtmlarr.push('</div>');
                if (g.hasChildren(o))
